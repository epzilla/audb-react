...
exports.inactivePlayers = function(req, res) {
  return Player.find({ active: false })
    .sort("lname")
    .exec(function(err, pls) {
      if (!err) {
        return res.json(pls);
...
exports.updatePlayer = function(req, res) {
  return Player.update({ _id: req.params.id }, req.body, function(
    err,
    updatedPlayer
  ) {
    if (err) {
...
exports.recruits = function(req, res) {
  return Recruit.find({ class: parseInt(req.params.year) }, function(
    err,
    recs
  ) {
    if (!err) {
...
exports.earlyEnrollees = function(req, res) {
  return Recruit.find({ class: req.params.year, earlyEnrollee: true }, function(
    err,
    recs
  ) {
    if (!err) {
...
exports.statsByopponent = function(req, res, next) {
  ...
  if (teams.indexOf("ALL-OPP") !== -1) {
    ...
    confs.indexOf("SEC East") !== -1 ||
    confs.indexOf("SEC West") !== -1
  ) {
    if (confs.indexOf("SEC East") !== -1) {
      _.remove(confs, function(c) {
        return c === "SEC East";
      });
      confs.push("SECEast");
    }
...
var filterResults = function(input, query) {
  ...
  if (query["homeAwayNeutral"]) {
    results = _.filter(
      results,
      r => r.homeAwayNeutral === query.homeAwayNeutral
    );
  }
      ...
      .exec(function(err, players) {
        ...
        if (req.body.insertFirst) {
          ...
          var max = players.reduce(function(prev, cur) {
            return prev.stringNum > cur.stringNum
              ? prev.stringNum
              : cur.stringNum;
          }, 1);
