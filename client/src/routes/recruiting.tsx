  ...
  render() {
    ...
    headers = this.columns.map(col => {
      ...
      if (this.state.sortCol === col.field) {
        ...
        arrow = this.state.sortAsc ? <span className="sort-arrow">▲</span> : <span className="sort-arrow">▼</span>;
      }

      return (<th className={className} onClick={() => self.sortCol(col.field)}>{col.name}{arrow}</th>);
    });
